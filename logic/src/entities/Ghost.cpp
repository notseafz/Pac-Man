...
Ghost::Ghost(float px, float py, float tw, float th, float delay)
    : DynamicEntity(px, py, tw, th), startX(px), startY(py), state(GhostState::Chase), frightenedTimer(0.0f),
...
float Ghost::getDistance(float x1, float y1, float x2, float y2) const { return std::abs(x1 - x2) + std::abs(y1 - y2); }
...
bool Ghost::handleExitLogic(float dt, float pacmanX) {
    ...
    if (std::abs(x - exitX) > moveStep) {
        if (x > exitX)
            x -= moveStep; // slide Left
        else
            x += moveStep; // slide Right
...
void Ghost::update(float dt, const std::vector<std::shared_ptr<Wall>>& walls, float targetX, float targetY, int pacDirX,
                   int pacDirY) {
    if (handleExitLogic(dt, targetX))
        return;
    ...
    if (!moved || isCentered(tolerance)) {
        ...
        if (!options.empty()) {
            ...
            for (auto& opt : options)
                if (std::abs(opt.dist - best.dist) < 0.001f)
                    ties.push_back(opt);
...
void Ghost::setSpawnDelay(float d) { spawnDelay = d; }
...
void OrangeGhost::update(float dt, const std::vector<std::shared_ptr<Wall>>& walls, float targetX, float targetY,
    ...
    if (!moved || isCentered(tolerance)) {
        ...
        int openPaths = 0;
        if (!willCollide(x + tileW, y, walls))
            openPaths++;
        if (!willCollide(x - tileW, y, walls))
            openPaths++;
        if (!willCollide(x, y + tileH, walls))
            openPaths++;
        if (!willCollide(x, y - tileH, walls))
            openPaths++;
        ...
        if (pickNew) {
            ...
            if (!willCollide(x + tileW, y, walls) && dirX != -1)
                valid.push_back({1, 0});
            if (!willCollide(x - tileW, y, walls) && dirX != 1)
                valid.push_back({-1, 0});
            if (!willCollide(x, y + tileH, walls) && dirY != -1)
                valid.push_back({0, 1});
            if (!willCollide(x, y - tileH, walls) && dirY != 1)
                valid.push_back({0, -1});
