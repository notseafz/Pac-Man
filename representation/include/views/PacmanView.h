...
namespace Representation {
    ...
    public:
        PacManView(std::shared_ptr<Logic::PacMan> m, Camera& cam, sf::Texture& tex,
                   std::vector<sf::IntRect> right, std::vector<sf::IntRect> left,
                   std::vector<sf::IntRect> up, std::vector<sf::IntRect> down)
        ...
        void update() override {
            ...
            if (dx == 1) currentAnim = &animRight;
            if (dx == -1) currentAnim = &animLeft;
            if (dy == 1) currentAnim = &animUp;
            if (dy == -1) currentAnim = &animDown;
